...
void main() {
  test('migrates learner state from schema version 2 to 7', () async {
    ...
    expect(await _tableExists(database, 'competency_attempts'), isTrue);
    expect(await _tableExists(database, 'competency_task_results'), isTrue);
    expect(await _tableExists(database, 'competency_gaps'), isTrue);
    expect(
      await _tableExists(database, 'competency_recovery_executions'),
      isTrue,
    );
  });
  ...
  test('migrates schema version 6 to competency persistence tables', () async {
    final executor = NativeDatabase.memory(
      setup: (database) {
        database
          ..execute('''
    ...
    final versionRow = await database
        .customSelect('PRAGMA user_version')
        .getSingle();
    final lessonRows = await database.customSelect('''
      SELECT attempt_id
      FROM lesson_attempts
      WHERE attempt_id = 'attempt.v6'
    ''').get();

    expect(versionRow.read<int>('user_version'), 7);
    expect(lessonRows.single.read<String>('attempt_id'), 'attempt.v6');
    expect(await _tableExists(database, 'competency_attempts'), isTrue);
    expect(await _tableExists(database, 'competency_task_results'), isTrue);
    expect(await _tableExists(database, 'competency_gaps'), isTrue);
    expect(
